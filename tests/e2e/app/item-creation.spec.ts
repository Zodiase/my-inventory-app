...
test.describe('User Story 1: Create and Organize Items', () => {
    ...
    test('T013b: Item appears in inventory list after creation', async ({ page }) => {
        const inventoryPage = new InventoryPage(page);
    ...
    test('T013e: View item details shows location breadcrumb trail', async ({ page }) => {
        const itemDetailPage = new ItemDetailPage(page);

        // Create a hierarchy: Kitchen > Cabinet > Shelf > Item
        const { kitchenId, cabinetId, shelfId } = await testData.createLocationHierarchy(page);
        ...
        const hasCabinet = await itemDetailPage.breadcrumbContains('Kitchen Cabinet');
    ...
    test('T013f: Verify all touch targets are 44×44px minimum', async ({ page }) => {
        const inventoryPage = new InventoryPage(page);
