...
test.describe("Touch Optimization - User Story 5", () => {
  ...
  test("T053b: Long-press on item reveals context menu", async ({ page }) => {
    ...
    const contextMenu = page.locator('[data-testid="context-menu"]').or(
      page.locator('text="View Details"').or(
        page.locator('text="Edit"').or(page.locator('text="Delete"')),
      ),
    );
  ...
  test("T053c: Pull-to-refresh works on item lists", async ({ page }) => {
    ...
    const scrollContainer = page.locator('[data-testid="items-list"]').or(
      page.locator("text=No items at this level").locator("..").locator(".."),
    );
   ...
   */
  test("T053e: Visual feedback on button press (iOS-style highlight)", async ({
    page,
  }) => {
    // Set viewport to iPhone size
    await page.setViewportSize({ width: 375, height: 667 });
    ...
    const pressedBg = await addButton.evaluate((el) =>
      window.getComputedStyle(el).backgroundColor,
    );
   ...
   */
  test("T053f: Keyboard does not obscure input fields (viewport adjusts)", async ({
    page,
  }) => {
    // Set viewport to iPhone size
    await page.setViewportSize({ width: 375, height: 667 });
  ...
  test("T053g: Double-tap prevention on submit buttons", async ({ page }) => {
    ...
    const itemCount = await page
      .locator("text=Double Tap Test Item")
      .count();
  ...
  test("T053h: Smooth scroll with momentum in long lists", async ({ page }) => {
    ...
    const scrollContainer = page.locator('[data-testid="items-list"]').or(
      page.locator("text=Scroll Test Item 1").locator("..").locator(".."),
    );
    ...
    const hasWebkitScrolling = await scrollContainer.first().evaluate((el) => {
      ...
      return (
        (style as unknown as { WebkitOverflowScrolling?: string })
          .WebkitOverflowScrolling === "touch" || style.overflowY === "auto"
      );
    });
    ...
    await expect(
      page.locator("text=Scroll Test Item 10"),
