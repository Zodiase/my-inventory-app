...
test.describe("User Story 2: Tag Items for Cross-Location Collections", () => {
  ...
  test("T034e: Rename tag and verify all tagged items updated", async ({
    page,
  }) => {
    const tagsPage = new TagsPage(page);
  ...
  test("T034f: Delete tag and verify removed from all items", async ({
    page,
  }) => {
    const tagsPage = new TagsPage(page);
    const inventoryPage = new InventoryPage(page);
  ...
  test("T034g: Tags are case-insensitive (reject duplicate)", async ({
    page,
  }) => {
  ...
  test("should find items across different locations by tag", async ({
    page,
  }) => {
    const tagsPage = new TagsPage(page);
